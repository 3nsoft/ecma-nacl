...
import * as sign from '../../lib/signing/sign';
...
describe(`sign module`, () => {
	...
	it(`sign function`, () => {
		...
		const expectedPKey = new Uint8Array(
		...
		const pair = sign.generate_keypair(keySeed);
		expect(bytesEqual(pair.pkey, expectedPKey)).toBe(true);
	
		// Testing of message signing");
		const m = asciiStrToUint8Array("testing\n");
	
		const signed_m = sign.sign(m, pair.skey, arrFactory);
		const result = sign.open(signed_m, pair.pkey, arrFactory);
		expect(result).not.toBeUndefined("FAILED signature verification.");
		expect(bytesEqual(result!, m)).toBe(true);
		
		// Testing of separated-signature functionality
		const sig = sign.signature(m, pair.skey, arrFactory);
		expect(bytesEqual(sig, signed_m.subarray(0,64))).toBe(true);
		expect(sign.verify(sig, m, pair.pkey, arrFactory)).toBe(
			true, "FAILED signature verification.");
		arrFactory.wipeRecycled();
	});

	it(`Testing for other key seeds`, () => {
		...
		seedAndPKeys.forEach((seedAndPk, i) => {
			...
			const expectedPKey = new Uint8Array(32);
