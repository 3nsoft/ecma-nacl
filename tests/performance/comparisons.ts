...
function boxEncryption(numOfRuns: number, msgKs: number): void {
	...
	testUtil.compare(pk1, box.generate_pubkey(sk1),
	...
	console.log("\tecma-nacl average for packing: "+
			diff.toFixed(3)+" milliseconds");
	...
	console.log("\tjs-nacl average for packing: "+
			diff.toFixed(3)+" milliseconds");
	
	testUtil.compare(cipher1, cipher2,
	...
	hrtimes = process.hrtime(startTime);
	diff = (hrtimes[0]*1e9 + hrtimes[1]) / numOfRuns / 1e6;
	console.log("\ttweetnacl average for packing: "+
		diff.toFixed(3)+" milliseconds");
...
function secretBoxEncryption(numOfRuns: number, msgKs: number): void {
	...
	var hrtimes = process.hrtime(startTime);
	diff = (hrtimes[0]*1e9 + hrtimes[1]) / numOfRuns / 1e6;
	console.log("\tecma-nacl average for opening: "+
			diff.toFixed(3)+" milliseconds");
	testUtil.compare(msg, recoveredMsg, "Message was incorrectly decrypted.");
	...
	hrtimes = process.hrtime(startTime);
	diff = (hrtimes[0]*1e9 + hrtimes[1]) / numOfRuns / 1e6;
	console.log("\tjs-nacl average for opening: "+
			diff.toFixed(3)+" milliseconds");
	testUtil.compare(msg, recoveredMsg, "Message was incorrectly decrypted.");
	...
	hrtimes = process.hrtime(startTime);
	diff = (hrtimes[0]*1e9 + hrtimes[1]) / numOfRuns / 1e6;
	console.log("\ttweet-nacl average for opening: "+
			diff.toFixed(3)+" milliseconds");
	testUtil.compare(msg, recoveredMsg, "Message was incorrectly decrypted.");
}
...
function sha512Hashing(numOfRuns: number, msgKs: number): void {
	...
	var hrtimes = process.hrtime(startTime);
	var diff = (hrtimes[0]*1e9 + hrtimes[1]) / numOfRuns / 1e6;
	console.log("\tecma-nacl average for hashing: "+
			diff.toFixed(3)+" milliseconds");
	...
	hrtimes = process.hrtime(startTime);
	diff = (hrtimes[0]*1e9 + hrtimes[1]) / numOfRuns / 1e6;
	console.log("\tjs-nacl average for hashing: "+
			diff.toFixed(3)+" milliseconds");
	
	testUtil.compare(hash1, hash2,
	...
	hrtimes = process.hrtime(startTime);
	diff = (hrtimes[0]*1e9 + hrtimes[1]) / numOfRuns / 1e6;
	console.log("\ttweetnacl average for hashing: "+
			diff.toFixed(3)+" milliseconds");
...
function signing(numOfRuns: number, msgKs: number): void {
	...
	var hrtimes = process.hrtime(startTime);
	var diff = (hrtimes[0]*1e9 + hrtimes[1]) / numOfRuns / 1e6;
	console.log("\tecma-nacl average for signing: "+
			diff.toFixed(3)+" milliseconds");
	...
	hrtimes = process.hrtime(startTime);
	diff = (hrtimes[0]*1e9 + hrtimes[1]) / numOfRuns / 1e6;
	console.log("\ttweetnacl average for signing: "+
			diff.toFixed(3)+" milliseconds");
	
	testUtil.compare(signature1, signedMsg2.subarray(0, 64),
	...
	hrtimes = process.hrtime(startTime);
	diff = (hrtimes[0]*1e9 + hrtimes[1]) / numOfRuns / 1e6;
	console.log("\tecma-nacl average for signature verification: "+
			diff.toFixed(3)+" milliseconds");
	if (!isSigOK) {
	...
	hrtimes = process.hrtime(startTime);
	diff = (hrtimes[0]*1e9 + hrtimes[1]) / numOfRuns / 1e6;
	console.log("\ttweetnacl average for signature verification: "+
			diff.toFixed(3)+" milliseconds");
...
function scryptKeyDerivation(logN: number, r: number, p: number): void {
	...
	// ecma-nacl scrypt
	var startTime = process.hrtime();
	var key1 = nacl.scrypt(passwd, salt, logN, r, p, resKeyLen, (p) => {});
	var hrtimes = process.hrtime(startTime);
	var diff = (hrtimes[0]*1e9 + hrtimes[1]) / 1e6;
	console.log("\tecma-nacl scrypt: "+
			diff.toFixed(3)+" milliseconds");
