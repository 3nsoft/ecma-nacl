 ...
 * test vectors from scrypt rfc https://tools.ietf.org/html/rfc7914
 */

import * as scrypt from '../../lib/scrypt/scrypt';
...
describe(`scrypt module`, () => {
	...
	it(`Testing scrypt with logN==4, r==1, p==1`, () =>{
		const P = asciiStrToUint8Array("");
		const S = asciiStrToUint8Array("");
		const logN = 4;
		const r = 1;
		const p = 1;
		const expectation = new Uint8Array([
			...
			0x38, 0xd1, 0x89, 0x06 ]);
		const hlen = expectation.length;
		const h = scrypt.scrypt(P, S, logN, r, p, hlen, (p) => {}, arrFactory);
		expect(bytesEqual(h, expectation)).toBe(true);
		arrFactory.wipeRecycled();
	});

	it(`Testing scrypt with logN==10, r==8, p==16`, () =>{
	...
	it(`Testing scrypt with logN==14, r==8, p==1`, () =>{
		const P = asciiStrToUint8Array("pleaseletmein");
		const S = asciiStrToUint8Array("SodiumChloride");
		const logN = 14;
		const r = 8;
		const p = 1;
		const expectation = new Uint8Array([
	...
	//
	// it(`Testing scrypt with logN==20, r==8, p==1`, () =>{
	// 	const P = asciiStrToUint8Array("pleaseletmein");
	// 	const S = asciiStrToUint8Array("SodiumChloride");
	// 	const logN = 20;
	// 	const r = 8;
