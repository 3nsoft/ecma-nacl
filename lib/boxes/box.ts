...
export function generate_pubkey(sk: Uint8Array,
	...
	if (!arrFactory) { arrFactory = new arrays.Factory(); }
 ...
 */
function packWithNonce(m: Uint8Array, n: Uint8Array,
		pk: Uint8Array, sk: Uint8Array, arrFactory?: arrays.Factory): Uint8Array {
	if (!arrFactory) { arrFactory = new arrays.Factory(); }
	var k = calc_dhshared_key(pk, sk, arrFactory);
 ...
 * Array is a view of buffer, which has 32 zeros preceding message bytes.
 */
function openWithNonce(c: Uint8Array,
		pk: Uint8Array, sk: Uint8Array, arrFactory?: arrays.Factory): Uint8Array {
	if (!arrFactory) { arrFactory = new arrays.Factory(); }
	var k = calc_dhshared_key(pk, sk, arrFactory);
 ...
 */
function makeEncryptor(pk: Uint8Array, sk: Uint8Array, nextNonce: Uint8Array,
	...
	, enc = sbox.formatWN.makeEncryptor(k, nextNonce, delta, arrFactory);
	arrFactory.wipe(k);
	return enc;
}
 ...
 */
function makeDecryptor(pk: Uint8Array, sk: Uint8Array,
		arrFactory?: arrays.Factory): Decryptor {
	if (!arrFactory) { arrFactory = new arrays.Factory(); }
	var k = calc_dhshared_key(pk, sk)
	, enc = sbox.formatWN.makeDecryptor(k, arrFactory);
	arrFactory.wipe(k);
	return enc;
}
