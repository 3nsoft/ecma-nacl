...
function testSingleChainHeader(test: nu.Test,
	...
	test.equals(writer.contentLength(), null);
	test.equals(writer.segmentsLength(), null);
	test.equals(writer.numberOfSegments(), null);
	writer.setContentLength(dataLen);
	test.ok(!writer.isEndlessFile());
	test.equals(writer.contentLength(), dataLen);
	...
	test.equals(writer.numberOfSegments(), fileSegments.length);
	...
	test.equals(offset, segmentsLen);
	...
	test.equals(reader.contentLength(), dataLen);
	test.equals(reader.segmentsLength(), segmentsLen);
	test.equals(reader.numberOfSegments(), numOfSegment);
...
//	var mkeyDecr = nacl.secret_box.formatWN.makeDecryptor(masterKey);
	...
	var reader = fkeyHolder.segReader(completeFile.subarray(segsEnd));
	test.ok(reader.isEndlessFile());
	test.equals(reader.contentLength(), null);
	test.equals(reader.segmentsLength(), null);
	test.equals(reader.numberOfSegments(), null);
	offset = xsp.SEGMENTS_OFFSET;
	var segInd = 0;
	var dataParts: Uint8Array[] = [];
	var decRes: { data: Uint8Array; segLen: number; };
