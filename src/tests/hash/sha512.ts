...
import * as sha512 from '../../lib/hash/sha512';
...
describe(`sha512 module`, () => {

	it(`Analog of tests/hash.c, with result printed in tests/hash.out`, () => {
		...
		const result = sha512.hash(x, arrFactory);
		expect(bytesEqual(result, expected)).toBe(true);
		arrFactory.wipe(result);
	});

	it(`Above test allowed for correct short vector's hashing, while incorrect long vector hashing. The following test is to remedy this.`, () => {
		...
		const expected = new Uint8Array(
			[ 0xe3, 0x47, 0x65, 0x5d, 0x31, 0xac, 0x3b, 0xfa, 0xe1, 0xd4,
			  ...
			  0x45, 0x8f, 0x40, 0x8b ]);
		const result = sha512.hash(x);
		expect(bytesEqual(result, expected)).toBe(true);
		arrFactory.wipe(result);
	});

	it(`Testing of sha512 hasher, with vector shorter than 128 bytes`, () => {
		...
		const expected = new Uint8Array(
			[ 0xe3, 0x47, 0x65, 0x5d, 0x31, 0xac, 0x3b, 0xfa, 0xe1, 0xd4,
			  ...
			  0x4f, 0x7f, 0xc5, 0x91, 0xa0, 0x1c, 0x60, 0xd2, 0xcb, 0x6f,
			  0x45, 0x8f, 0x40, 0x8b ]);
		const hasher = sha512.makeHasher(false);
	...
	it(`Testing of sha512 hasher, with vector equal to 128 bytes`, () => {
		...
		const expected = new Uint8Array(
			[ 0xbd, 0xb1, 0x1, 0x77, 0x73, 0x6b, 0x2b, 0xa2, 0x64, 0xac,
			  ...
			  0x35, 0x48, 0x23, 0xba ]);
		const hasher = sha512.makeHasher();
	...
	it(`Testing of sha512 hasher, with vector longer than 128 bytes`, () => {
